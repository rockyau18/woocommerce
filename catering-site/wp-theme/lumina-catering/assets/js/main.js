...
if (form) {
  form.addEventListener("submit", (e) => {
    ...
    if (window.luminaAjax) {
      data.append("action", "lumina_inquiry");
      data.append("nonce", luminaAjax.nonce);
      if (submit) submit.disabled = true;
      fetch(luminaAjax.url, { method: "POST", body: data, credentials: "same-origin" })
        .then((res) => res.json())
        .then((json) => {
          const ok = !!(json && json.success);
          const msg = ok
            ? translations[currentLang]["form.success"]
            : (json && json.data && json.data.message) || "Please try again.";
          if (notice) {
            notice.hidden = false;
            notice.className = "form-notice " + (ok ? "success" : "error");
        ...
        .catch(() => {
          const msg = "Please try again or WhatsApp us.";
          if (notice) {
            notice.hidden = false;
